...
from logutil import info, debug, warn
...
def summarize_with_openai(text: str, title: str) -> str:
    ...
    try:
        ...
        info(
            f"Docs: summary complete for {title} "
            f"(chars={len(summary)})"
        )

        dt = time.time() - t0
        info(
            f"OpenAI: summary returned for {title} "
            f"in {dt:.1f}s ({len(summary)} chars)"
        )
